...
int main(int argc, char* argv[])
{
  ...
  if (argc != 5) // check number of arguments
    ...
    std::cerr << "Usage: " << argv[0] <<  "<hostname>  <port> <track0 file> <track1 file>" << std::endl;
    std::cerr << "    <hostname>    : IP or host name"                    << std::endl;
    std::cerr << "    <port>        : Port # (18944 in Slicer default)"   << std::endl;
    std::cerr << "    <track0 file> : Input file for tracking #0"   << std::endl;
    std::cerr << "    <track1 file> : Input file for trakcing #1"   << std::endl;
    exit(0);
    }

  char*  hostname = argv[1];
  int    port     = atoi(argv[2]);
  std::string fname0 = argv[3];
  std::string fname1 = argv[4];

  std::ifstream ifs0 (fname0, std::ofstream::out);
  std::ifstream ifs1 (fname1, std::ofstream::out);
